 ...
 *чтобы передать ему какую-либо информацию, нужно записать ее в специальный
 *адрес. Например, чтобы вывести символ на экран, мы записывали его в адрес
 *0xb8000. Если девайс использует I/O ports, то для взаимодействия с ним
 *		используются инструкции ассемблера in и out.
 *
 *       Чтобы работать с дейвасом, нам достаточно знать что в нем есть некий
 *       контролирующий чип, который делает всю грязную работу за нас и
 *позволяет процессору взаимодействовать с напрямую с железом дейваса. У такого
 *чипа есть регистры в которые можно что-то записывать или что-то из них читать,
 *и значение этих регистров указывет чипу что делать.
 *
...
unsigned char port_byte_in(unsigned short port) {
    ...
    /* : "d" (port)			- Загрузить port в регистр EDX (extended
     * DX: 32b) */
