...
u32 pkmalloc_a(u32 sz) { return pkmalloc_internal(sz, 1, 0); }
...
u32 pkmalloc(u32 sz) { return pkmalloc_internal(sz, 0, 0); }
...
void page_fault(registers_t regs) {
  ...
  printf(") at %x\n\n", faulting_address);
  printf("PRESENT: %d | RW: %d | US: %d | RESERVED: %d | ID: %d", present, rw,
         us, reserved, id);
  kprint("\n");
}
...
page_t *get_page(u32 address, int make, page_directory_t *dir) {
  ...
  if (dir->tables[table_idx]) { // Если таблица уже назначена
    ...
    dir->tables[table_idx] =
        (page_table_t *)pkmalloc_ap(sizeof(page_table_t), &tmp);
