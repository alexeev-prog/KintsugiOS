...
*	Description: Страничная организация памяти (paging) — это схема управления
*  памятью, которая вводит концепции **_логических адресов_**
...
/* Внутренняя функция выделения памяти с учетом выравнивания и физического адреса */
...
void initialise_paging() {
    ...
    // Identity mapping (phys addr = virt addr) от 0x0 до конца используемой памяти
    // Используем while для вычисления free_mem_addr на лету в теле цикла
    int i = 0;
    while (i < free_mem_addr)
    {
        // Код ядра доступен для чтения, но не для записи из пользовательского пространства
