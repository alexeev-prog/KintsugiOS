...
void *kmalloc(u32 size) {
  ...
  // Если дошли сюда, значит, подходящего блока не найдено -> пробуем расширить
  // кучу
...
void *krealloc(void *ptr, u32 size) {
  ...
  if (block->size >= size) {
    ...
    if (block->size - size >= sizeof(mem_block_t) + BLOCK_SIZE) {
      mem_block_t *new_block =
          (mem_block_t *)((u32)block + sizeof(mem_block_t) + size);
      ...
      for (u32 page = start_page; page < end_page; page += PAGE_SIZE) {
        ...
        if (page_entry && page_entry->present) {
          ...
          while (check) {
            ...
            if (!check->is_free && check_start < page + PAGE_SIZE &&
                check_end > page) {
              page_is_free = 0;
              break;
            }
...
void kmemdump() {
  ...
  printf("Heap pages: PRESENT=%d  |  NOT PRESENT=%d", present_pages_count,
         not_present_pages_count);
  ...
  while (current) {
    ...
    for (u32 page_addr = start_page; page_addr < end_page;
         page_addr += PAGE_SIZE) {
